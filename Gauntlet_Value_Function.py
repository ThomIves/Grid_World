from Gauntlet_Grid_Visualization import Grid_Visualization as GV
...
class Grid_Data:
    def __init__(self, grid_dict={}, rows=0, cols=0, percent_walls=0,
        ...
        self.rewards_D = {}
        self.trans_probs_D = {((1, 2), 'up', (0, 2)): 0.3,
                              ((1, 2), 'up', (1, 3)): 0.7}
        self.policy = {}
        self.actions_probs = {}
        self.max_acts = {}
    ...
    def __get_move_from_trans_probs_D__(self, current_loc, move_str):
        new_loc_D = {}
        cdf = 0
        coin = random.uniform(0, 1)

        for key in self.trans_probs_D.keys():
            # print(current_loc, move_str, key)
            if current_loc == key[0] and move_str == key[1]:
                new_key = (key[0], key[1])
                if new_key not in new_loc_D.keys():
                    new_loc_D[new_key] = []
                prob_range = [cdf]
                cdf += self.trans_probs_D[key]
                prob_range.append(cdf)
                prob_range = tuple(prob_range)

                new_loc_D[new_key].append((key[2], prob_range))

        for key in new_loc_D.keys():
            if len(new_loc_D[key]) >= 2:
                print('\t', coin, new_loc_D)
                for a_tuple in new_loc_D[key]:
                    if a_tuple[1][0] <= coin <= a_tuple[1][1]:
                        print('\t', '\t', a_tuple)
                        return a_tuple[0]
            else:
                # print('\t', coin, new_loc_D, 0)
                return 0

    def __get_loc_for_move__(self, current_loc, move_str):
        new_loc = self.__get_move_from_trans_probs_D__(current_loc, move_str)
        if new_loc:
            return new_loc
    ...
    def __episode_wrap_up__(self):
        ...
        # print()
        # pp.pprint(self.trans_probs_D)
        # print()
        print()

        self.epsilon -= 0.01  # self.gamma
...
gd = Grid_Data(grid_dict=grid_dict, epsilon=0.9)  # rows=4, cols=6, max_steps=800)
